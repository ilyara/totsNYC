...
def walk(x, level=0, key='topLevel', parent_type=None):
  ...
  if isinstance(x, dict):
    if parent_type == dict:
      print "%sobject(of type %s): %d keys" % (indent, key, len(x.keys()))	
      objects[key] = x.keys()
    else:
      print "%srecord(in %s): %d keys" % (indent, key, len(x.keys()))
      tables[key] = x.keys()
    [walk(x[k], level+1, k, dict) for k in x.keys()]
  elif isinstance(x, list):
    print "%sreference table(%s): %d items" % (indent, key, len(x))
